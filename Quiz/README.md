## How to Run

This is a simple C++ quiz program that runs in the terminal or an IDE.

### Steps

1. **Clone or download** this repository.
2. Open the `quiz.cpp` file in a C++ IDE (like Code::Blocks, Visual Studio, or VSCode with C++ extensions).
3. **Compile the program**:

   ```bash
   # Linux / Mac / Windows (if using terminal)
   g++ -o quiz main.cpp
Run the program:

# Linux / Mac
./quiz

# Windows
quiz.exe
Notes
The program is case-insensitive; you can type a or A for your answer.

Make sure your terminal/IDE is pointing to the directory containing quiz.cpp before running the compile command.

If you have multiple .cpp files in the future, you can compile them together:

g++ -o quiz quiz.cpp min.cpp utils.cpp
